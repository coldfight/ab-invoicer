...
// @todo: InvoiceItem should NOT be a list. Remove all list related code here.
...
func newListKeyMap() *listKeyMap {
	return &listKeyMap{
		...
		back: key.NewBinding(
			key.WithKeys("esc"),
			key.WithHelp("esc", "back to invoice list"),
		),
	}
}
...
func New(invoiceNumber models.InvoiceNumber, windowSize tea.WindowSizeMsg) Model {
	...
	// Retrieve the invoice by its invoice number
	...
	menuList.Title = fmt.Sprintf("Invoice %s", invoiceNumber.Padded())
